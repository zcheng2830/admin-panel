...
- `/admin/captions` read-only caption explorer with `image_id` filtering and per-page caption quality summaries
- Pagination controls on major data pages (`users`, `images`, `captions`, and `/admin/[resource]`)
- Additional resource sections under `/admin/*`:
  - Read-only: `reported_captions`, `humor_flavors`, `humor_flavor_steps`, `caption_requests`, `llm_prompt_chains`, `llm_responses`
  - Read/update: `humor_mix`
  - CRUD: `terms`, `caption_examples`, `llm_models`, `llm_providers`, `allowed_signup_domains`, `whitelisted_emails`
